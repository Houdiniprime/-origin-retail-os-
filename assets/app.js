(function () {
  ...
  const USERS = [
    ...
    { id: "gm", name: "Gerante Meshes", role: "manager", code: "2201", shop: SHOPS[0], email: "gerante.meshes@originretail.com", phone: "+237600000002" },
    { id: "gp", name: "Gerante Packaging", role: "manager", code: "2202", shop: SHOPS[1], email: "gerante.packaging@originretail.com", phone: "+237600000003" },
    { id: "cm1", name: "Caissiere Meshes 1", role: "cashier", code: "1101", shop: SHOPS[0], phone: "+237600000004" },
    { id: "cm2", name: "Caissiere Meshes 2", role: "cashier", code: "1102", shop: SHOPS[0], phone: "+237600000005" },
    { id: "cp1", name: "Caissiere Packaging 1", role: "cashier", code: "1201", shop: SHOPS[1], phone: "+237600000006" },
    { id: "cp2", name: "Caissiere Packaging 2", role: "cashier", code: "1202", shop: SHOPS[1], phone: "+237600000007" }
  ];
  const ICONS = {
  ...
  const seed = {
    auth: null,
    theme: "light",
    settings: { boutique: "Origin Retail OS", gmail: "boutique@example.com", users: USERS, lowStock: 5 },
    products: [
      { id: "p1", sku: "MSH-PER-001", name: "Perruque lace premium", category: "Meshes", shop: SHOPS[0], qty: 16, cost: 11500, price: 22000, photo: "" },
      { id: "p2", sku: "MSH-BON-002", name: "Bonnet wig cap", category: "Accessoires cheveux", shop: SHOPS[0], qty: 42, cost: 600, price: 1500, photo: "" },
      { id: "p3", sku: "MSH-COL-003", name: "Colle lace forte", category: "Accessoires cheveux", shop: SHOPS[0], qty: 23, cost: 1800, price: 3500, photo: "" },
      { id: "p4", sku: "MSH-BAN-004", name: "Bande elastique pose", category: "Accessoires cheveux", shop: SHOPS[0], qty: 7, cost: 900, price: 2500, photo: "" },
      { id: "p5", sku: "PKG-POT-011", name: "Pot creme 100 ml", category: "Packaging cosmetique", shop: SHOPS[1], qty: 110, cost: 220, price: 550, photo: "" },
      { id: "p6", sku: "PKG-FLC-012", name: "Flacon serum 30 ml", category: "Packaging cosmetique", shop: SHOPS[1], qty: 85, cost: 260, price: 650, photo: "" },
      { id: "p7", sku: "PKG-ETQ-013", name: "Etiquettes waterproof", category: "Packaging cosmetique", shop: SHOPS[1], qty: 240, cost: 35, price: 120, photo: "" },
      { id: "p8", sku: "PKG-SPA-014", name: "Spatule cosmetique", category: "Accessoires cosmetiques", shop: SHOPS[1], qty: 32, cost: 90, price: 250, photo: "" }
    ],
    clients: [
    ...
    return '<main class="login-page"><section class="login-card">' +
      ...
      (loginTab === 'login' ? (
        ...
        '<label class="register-shop-field" style="display:' + (state._registerRole === 'cashier' ? 'none' : 'block') + '">Boutique<select name="shop"><option value="all">Toutes boutiques</option>' +
          SHOPS.map(function(s) { return '<option value="' + esc(s) + '">' + esc(s) + '</option>'; }).join('') +
        '</select></label>' +
    ...
    return `<div class="app-shell" data-theme="${state.theme || 'light'}">
      ...
      <main>
        <header class="topbar"><div><h1>${title()}</h1><p>${subtitle()}</p></div><div class="actions">
          ...
          <select data-shop>${["all", ...SHOPS].filter((s) => state.auth.shop === "all" || s === state.auth.shop).map((s) => `<option ${shopFilter === s ? "selected" : ""} value="${esc(s)}">${s === "all" ? "Toutes boutiques" : esc(s)}</option>`).join("")}</select>
  ...
  function showEditProductModal(id) {
    ...
    panel.innerHTML = `<div class="modal-box">
      ...
      <form id="editProdForm">
        ...
        <div class="grid two">
          <label>Boutique<select name="shop">${SHOPS.map(s => `<option ${p.shop === s ? 'selected' : ''}>${esc(s)}</option>`).join('')}</select></label>
    ...
    panel.innerHTML = `<div class="modal-box">
      ...
      <form id="userForm">
        ...
        <div class="grid two">
          ...
          <label>Boutique<select name="shop"><option value="all" ${existing?.shop === 'all' ? 'selected' : ''}>Toutes</option>${SHOPS.map(s => `<option value="${esc(s)}" ${existing?.shop === s ? 'selected' : ''}>${esc(s)}</option>`).join('')}</select></label>
        </div>
  ...
  function transferStockAdvanced() {
    const html = `<div class="panel">
      ...
      <div class="grid two" style="margin-top:8px">
        <select id="transferFrom" required><option value="">De (boutique source)</option>${SHOPS.map(s => `<option>${esc(s)}</option>`).join('')}</select>
        <select id="transferTo" required><option value="">Vers (boutique destination)</option>${SHOPS.map(s => `<option>${esc(s)}</option>`).join('')}</select>
      </div>
